...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (!data.has(uid)) data.set(uid, { title: 'Tytuł', desc: 'Opis', color: 0x4974FF, ts: false });
    ...
    if (interaction.isButton()) {
        if (interaction.customId === 'btn_download') {
            ...
            return interaction.reply({ content: `\`\`\`Tytuł:\`\`\`\n${d.title || ''}\n\n\`\`\`Treść:\`\`\`\n${d.desc || ''}\n\n\`\`\`Kolor:\`\`\`\n#${d.color ? d.color.toString(16) : ''}`, ephemeral: true });
        }
    ...
    if (interaction.isModalSubmit()) {
        ...
        if (interaction.customId === 'm_color') d.color = parseInt(interaction.fields.getTextInputValue('hex').replace('#', ''), 16) || 0x4974FF;
        ...
        const eb = new EmbedBuilder().setTitle(d.title || null).setDescription(d.desc || null).setColor(d.color);
        if (d.author) eb.setAuthor({ name: d.author, iconURL: d.auth_i || null });
        if (d.footer) eb.setFooter({ text: d.footer, iconURL: d.foot_i === 'server' ? interaction.guild.iconURL() : (d.foot_i || null) });
